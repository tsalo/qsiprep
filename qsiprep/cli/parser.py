...
def _build_parser():
    ...
    verstr = "QSIPrep v{}".format(config.environment.version)
    ...
    parser.add_argument(
        ...
        action="store",
        help=(
            "the root folder of a BIDS valid dataset (sub-XXXXX folders "
    ...
    g_bids.add_argument(
        ...
        help=(
            ...
            "https://qsiprep.readthedocs.io/en/"
            f"{currentv.base_version if is_release else 'latest'}/faq.html#"
            "how-do-I-select-only-certain-files-to-be-input-to-QSIPrep"
        ),
    )
    g_bids.add_argument(
        "-d",
    ...
    if latest is not None and currentv < latest:
        print(
            f"""\
You are using qsiprep-{currentv}, and a newer version of qsiprep is available: {latest}.
Please check out our documentation about how and when to upgrade:
https://qsiprep.readthedocs.io/en/latest/faq.html#upgrading""",
            file=sys.stderr,
        )
    ...
    if _blist[0]:
        ...
        print(
            f"""\
WARNING: Version {config.environment.version} of qsiprep (current) has been FLAGGED
(reason: {_reason}).
...
mention this particular variant of qsiprep listing the participants for which it was \
applied."""
        )
    ...
    if config.execution.qsiprep_dir is None:
        config.execution.qsiprep_dir = output_dir
    ...
    if output_dir == bids_dir:
        rec_path = bids_dir / "derivatives" / f"qsiprep-{version.split('+')[0]}"
        parser.error(
    ...
    if not opts.skip_bids_validation:
        from qsiprep.utils.bids import validate_input_dir
